...
//*******************************
#define	canal_an1 pin_a0//canales anal�gicos
...
#define clk pin_c7  //reloj para comunicaci�n con sht
#define pulso_muestreo pin_b0//interrupci�n externa cada segundo
...
//******************************************************//
//** Declaraci�n de Variables de lecturas instantaneas**//
//******************************************************//
...
//Guarda lecturas de direcci�n y magnitud de V2xe
...
//    //Envia caracter de inicio de transmisi�n y la id de la estaci�n
...
//    //Envia caracter de fin de transmisi�n
//       printf(envia_rf,"\n");
//
   if (enviar_datos)
   {
      //      caracter_inicio=0x21;
      //Envia caracter de inicio de transmisi�n y la id de la estaci�n
      printf(envia_rf,"%c%c",0x21,id_estacion);
      //Envia fecha y hora
      //++++   printf(envia_rf,"%2x/%2x/%2x %2x:%2x:%2x ",dia,mes,anio,horas,minutos,segundos);
      //Envia los datos de los sensores integrados al sistema
      printf(envia_rf,"%05.1f %04.1f %06.1f %05.1f ",temp_en_c,humedad_comp,presion,direccion);
      //Envia datos del canal de pulsos 1
      printf(envia_rf,"%04.1f ",velocidad);
      printf(envia_rf,"%06.1f",altura_relativa);
      printf(envia_rf," %05Lu ",dir_mem);
      printf(envia_rf,"0 ");
      printf(envia_rf,"%01.2f ",bateria);
      //Envia caracter de fin de transmisi�n
      printf(envia_rf,"\n");
      enviar_Datos=false;
   }      
...
//***********************************//
        ...
        set_timer0(0);
	velocidad=no_pulsos*0.0589; //para constante=17 (5 helices)
//	velocidad=no_pulsos*0.0667; //para constante=15
...
void WRITE_FLOAT_EXT_EEPROM( long int n, float data) {
   ...
   for (i = 0; i < 4 ; i++)
     ...
     write_ext_eeprom(i + n, *((int *)&data + i));
...
float READ_FLOAT_EXT_EEPROM( long int n) {
   ...
   for (i = 0; i < 4; i++)
     ...
     *(((int *)&data) + i) = read_ext_eeprom(i + n);
...
//                           i2c_write(anio);   //Cofigura a�o
//                           i2c_write(0b00010000);   //Saca la se�al del oscilador a 1hz
...
output_low(indicador);
   //   if (caracter_r=='s')      //Si hay conexi�n
     // {                     //REaliza esta rutina
         //Lcd_putc("\nPC encontrada...");
...
//
                    ...
                    //envia presion
                    presion=read_float_ext_eeprom(dir_mem);
                    printf(miputc,"%06.1f ",presion);
                     dir_mem=dir_mem+4;

                     //envia direcci�n
                     printf(miputc,"%3.1f ",read_float_ext_eeprom(dir_mem));
                     dir_mem=dir_mem+4;
...
//envia altura
               ...
               break;
         case 'T': 
    //Si se recibe "T" se configura la estaci�n
    //lcd_putc("\f*Configurando*");

      //***************************************//
      //***************************************//
      //*** Configuraci�n de la estaci�n  *****//
      //***************************************//
      //***************************************//
...
//Direcci�n de inicio de datos 0x20
